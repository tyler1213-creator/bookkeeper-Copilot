...
在 Case Judgment 与 JE Generation Node 之间设一个确定性代码校验步骤：将 Case Judgment 输出的 COA 科目与该客户 COA 全集匹配——命中则放行进入 JE 构造，未命中则打回。该步骤是纯确定性代码、不做会计判断；打回后的详细错误处理 / 纠错机制 MVP 阶段不细化（见缺口地图）。
...
**浮现的新 open boundary：**
CJ → JE Generation 之间 COA 校验节点的归属、打回后的错误处理 / 纠错流程与 exact contract，MVP 阶段不细化，留缺口地图。
...
- **兜底定位（恒有归处、不拒收）**：CJ 是整个系统的兜底与沟通桥梁。任何到达 CJ 的交易都必有归处——要么 High Confidence Classification，要么 Pending 交人类会计师。CJ 不「拒收」交易、不产生无归处的孤儿；其输出恒为 2.4 的两类之一。
...
- `BK_Copilot/workflow_nodes/case_judgment_node/01_functional_intent.md:定位 / 排除范围` 与 `02_logic_and_boundaries.md:边界` → `CJ 是系统兜底桥梁，恒输出 High Confidence 或 Pending、不拒收交易、不产生无归处孤儿。CJ 为 runtime 逐笔节点，不参与 onboarding / 历史回填；pending 经会计师解决的交易不重入 CJ。结构性无效输入的拒绝与否、无解交易最终归处、错误纠错机制 MVP 阶段不设计，见缺口地图。`
...
- **O3（触发 + 无效 / 被阻断 handoff 行为）已处理**：触发条件（非结构性 + deterministic path 未完成）属工作流接线，已散见依赖。CJ 定位为系统兜底桥梁，恒输出 High Confidence 或 Pending、不拒收交易（见 2.18）；结构性无效 / 损坏输入是否拒绝、被 CJ 与上游均无解交易的最终归处、纠错 candidate signal 发出者与纠错流程，均未设计，MVP 阶段不写入，DEFERRED 至缺口地图，待实战检验后再定。
