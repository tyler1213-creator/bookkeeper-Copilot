...
3. 防止 New system node/stage 文档被默认视为自动有效、当前 authority 或 implementation-ready contract
4. 准备从核心产品目标出发的组件级审计
...
- 根部治理文档指向真实存在的仓库路径
- Old system 文档被明确标记为审计对象，而不是历史意图权威、标准或 baseline
- New system 文档被明确标记为审计对象，而不是已证明的答案、当前 authority 或实现合同
- 核心产品目标被记录为审计标准
...
- 上游 / 下游依赖关系可见
- New system / Old system 文件都只被标记为审计对象，而不是 authority、baseline requirements 或正确性证明
...
- New system / Old system 原始材料不会被改写成当前 authority
