...
1. 先 map `BK_Copilot/` 当前正式草案与 active 治理材料，再评判它
2. 防止 Old-System 材料变成虚假的评判标准、历史意图权威或 baseline
3. 防止已作废的 New System 文档被继续视为审计目标、当前 authority 或 implementation-ready contract
4. 准备从核心产品目标出发的组件级审计
...
- 根部治理文档指向真实存在的仓库路径
- Old-System 文档被明确标记为授权读取历史材料，而不是审计对象、历史意图权威、标准或 baseline
- New System 文档被明确标记为已作废且不再作为审计目标，而不是已证明的答案、当前 authority 或实现合同
- New System / Old-System 文档被明确设为未经用户同意不得读取
- 核心产品目标被记录为审计标准
...
- 上游 / 下游依赖关系可见
- New System / Old-System 文件不进入默认材料盘点；如经用户授权读取，也只作为历史 source text / 追溯背景，而不是 authority、baseline requirements 或正确性证明
...
- 编辑范围限定在用户已确认的结论内
- 修订后形成的正式草案或用户确认结论才可能成为 active baseline
- New System / Old-System 原始材料不会被改写成当前 authority；New System 也不会被恢复为审计目标
...
- 哪些历史材料或待审对象中的能力主张，经独立论证后仍服务核心产品目标？
- 哪些当前抽象让设计更难审计或控制？
- 什么审计顺序能最快暴露最大结构风险？
