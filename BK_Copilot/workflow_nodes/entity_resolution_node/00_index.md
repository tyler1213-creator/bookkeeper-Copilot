...
- Entity Resolution 判断为 `new_stable_entity`（新稳定实体）时，可以同步写入 Entity Log，不需要 governance approval（治理批准）；写入内容限于 entity 本体，不写 Alias（别名）、confirmed role（已确认角色）、automation policy（自动化策略）或 rule（规则）。
