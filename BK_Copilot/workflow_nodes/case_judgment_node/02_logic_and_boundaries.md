...
| Entity Log | identity basis、identity risk flags、`automation_policy` | 判断身份可用性和 entity-level automation boundary | Entity Log 不是 classification memory；CJ 只读已落到 CJ 可见标记上的 policy / risk，不另读 Governance Log 原始账本 |
...
Case Log `use_level` 必审 / exception-only / 失效、negative case，或 Entity Log `automation_policy` 收紧，属于硬权限封顶，由代码转 Pending，不作为可被模型权衡后越过的普通事实。
...
- stable-linked 且 finalized 的 High Confidence 交易可形成 reusable case 语义，进入 Case Log 的资格仍由 Case Log / finalization proof 约束；CJ 只提供 case memory update candidate，不授权写入。
...
- `case_memory_update_candidate`：仅 High Confidence 自动路径产生，consumer 是 Case Memory Update Node。它表达 CJ 判断派生的可学习 case 内容，例如 evidence condition、context note、use_level 倾向或 risk hints；必须等交易 finalized 后凭 `transaction_log_ref` 或等价 finalization proof 处理。
...
- active rule。
- automation policy。
- entity / alias / role / rule / automation-governance candidate。
- 交易集合或 split 子交易。
...
- Entity Log `automation_policy` 收紧、identity risk flags、Case Log `use_level` 必审 / 不可复用 / 失效等已结构化标记是否命中 hard block。
...
- 最终路由：High Confidence 经 COA 校验进入 JE Generation Node；Pending 交 Coordinator；case memory update candidate 交 Case Memory Update Node。
...
- 越过 ER `unknown`、Entity Log policy / risk、Case Log `use_level` 等 hard block。
- 创建、修改或授权写入 durable memory。
- 创建 entity / alias / role / rule / automation-governance candidate。
...
- automation policy 的 upgrade / relaxation；系统自动收紧也必须保留 governance visibility。
...
本节点对下游只声明两类分类输出：High Confidence Classification 与 Pending。`case_memory_update_candidate` 是非分类候选信号，不是第三种分类状态。
...
| `case_memory_update_candidate`（非分类候选信号） | High Confidence 自动路径产生的可学习 case 内容候选，供 finalized 后进入 Case Log 写入流程。 | Case Memory Update Node | Case Memory Update Node 在交易 finalized 后凭 `transaction_log_ref` 或等价 finalization proof 处理写入资格。 | 不代表 Case Log 已写入，不代表 CJ 授权写入，不代表 `confirmed_by=accountant`，不代表 entity / alias / rule / automation-governance candidate。 |
...
1. 第 0 层：硬权威阻断。包括 ER `unknown`、Entity Log `automation_policy` 收紧、Entity Log 身份级 risk flags、Case Log `use_level` 必审 / 不可复用 / 已失效。命中即封顶为 Pending，当前证据再强也不能越过。CJ 不另读 Governance Log / Rule Log 原始账本。
...
- case memory update candidate 的 runtime refs；exact schema 留 L3。
...
| 身份类候选 / 多类治理候选由 CJ 直接产出 | 单次 CJ runtime judgment 不能越权进入身份、规则或治理 authority；CJ 只有 case memory update candidate 一个长期记忆出口 |
...
- CJ 不批准 entity merge / split、rule promotion、automation policy upgrade / relaxation 或 governance event。
- accountant 必须决定 Pending 的最终会计处理、个人 vs 公司立场、correction，以及某次说明是否应沉淀为长期 memory / rule / policy。
- governance 必须批准长期 authority 变化；系统自动收紧 automation policy 也必须保留 governance visibility。
- CJ 的唯一长期记忆候选出口是 High Confidence 路径的 `case_memory_update_candidate`，consumer 为 Case Memory Update Node。
...
4. Entity Log `automation_policy`、Case Log `use_level` / `confirmed_by` exact enum。
...
14. `case_memory_update_candidate` exact schema、Case Memory Update Node 写入机制、Case Log 与 Transaction Log finalization trigger order。
