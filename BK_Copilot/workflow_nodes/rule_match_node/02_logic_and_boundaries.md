...
- handoff 明确表明该交易已在上游 eligibility router 放行（force_pending 未命中）、可进入 RuleMatch。
...
- 桶 B：放行 / 控制基础。必须提供投影自上游 eligibility router 的 `eligibility` 语义，表明该交易已放行进入 rule-based automation（force_pending 命中即在 ER→RM router 改道 Case Judgment / Pending、不进 RuleMatch）；force_pending / promotion_lock 的 authority 仍在 EntityLog，RuleMatch 不读。
- 桶 C：客观交易事实。必须提供 transaction_id（锚）、direction、amount、transaction_date、currency 等客观维度作匹配。evidence_refs 只随 trace / 审计携带、不作匹配条件；raw description 同样只随 trace、不作匹配条件。（`evidence_condition`、`transaction_type` / `objective_tags` 经本轮判定 RuleMatch 不需要、已删——证据充分性无稳定衡量标准、后两者全库无产出方；见 Decisions 与 `L3_schema/RM_Schema/Rule_Match_L3_Schema.md`。）
...
| [不读] EntityLog | 不直接读取；RuleMatch eligibility（force_pending 已在上游 ER→RM router 过滤）经 ER handoff 或等价 workflow handoff 投影进入 | 避免 RuleMatchNode 二次解释 entity eligibility | EntityLog 仍是 force_pending / promotion_lock / identity 的 source of truth，但不是本节点直接读取对象 |
...
- RuleMatch 命中的交易应由后续 finalization 路径写入 TransactionLog。可持久化的语义包括当前交易事实、命中的 approved accounting treatment、StableEntity ref（= `entity_id`）、`rule_id` 与 `rule_match` source（= `classified_by = rule_match`，见 Decisions D11）；有效性来自 RuleLog 的 active approved rule authority 与 finalization 路径对本次交易完成状态的认定。
...
- 当前 handoff 是否同时具备 StableEntity、上游 eligibility 放行语义和客观交易事实。
...
| --- | --- | --- | --- | --- |
| `rule_hit` | 当前交易命中一条 active approved rule。输出 approved accounting treatment 与 rule hit context；最终分类 = 命中 rule 的 approved treatment + 桶 C 交易事实。桶 C 必要但不充分，分类“答案”来自 RuleLog。该输出是高权威（因 rule 已事前批准）的分类来源。 | JE Generator；后续 finalization / audit / learning | JE Generator 用 approved accounting treatment + 当前交易事实构造并校验 JE。后续 finalization / audit / learning 消费 rule-hit handoff 追溯 StableEntity ref（= `entity_id`）与 `rule_id`。 | 不代表 JE 已成功生成、TransactionLog 已 finalized、CaseLog 已写入、accountant 已完成本次交易最终审核，或本节点已持久化任何记录。 |
...
- StableEntity ref（= `entity_id`）。
- `rule_id`（命中规则的稳定 id；不钉版本，见 Decisions D9。原称 “Rule ref”）。
- approved accounting treatment。
- `rule_match` source（落为 `classified_by = rule_match`：本笔分类**由谁 / 什么做出** = 规则匹配；`classified_by` 与 `confirmed_by`〔最终会计分类结果的确认 / 审计，归 Transaction Log〕分离，见 Decisions D11）。

该 handoff 只作为 runtime handoff 交给后续 finalization / audit / learning / review，不由 RuleMatchNode 持久化。exact field schema、refs 形态和 enum 留 Stage 3。（L3 字段命名见 `L3_schema/RM_Schema/Rule_Match_L3_Schema.md` 与 Decisions D8/D9/D11：结局字段 `rule_match_outcome`，命中引用 `rule_id`，分类做出方 `classified_by`。）
...
- authority 顺序：RuleLog 是 rule payload 与 active approved status 的 authority；EntityLog 是 force_pending / promotion_lock / identity 的 source of truth，但只经 handoff 投影进入本节点。
...
- StableEntity ref（= `entity_id`）。
- `rule_id`（命中规则的稳定 id，见 Decisions D9；原称 “Rule ref”）。
- approved accounting treatment。
- `rule_match` source（= `classified_by = rule_match`，见 Decisions D11）。
...
RuleMatchNode 的可追溯性来自 rule-hit handoff：后续 TransactionLog / CaseLog / Governance review 应能通过其中的 StableEntity ref（= `entity_id`）与 `rule_id` 还原本次自动分类使用的 rule authority。
...
1. force_pending 中“哪种取值 = 上游 router 改道、不放行 rule-based automation”的 exact 取值集合 / 语义尚未冻结；归 L3，需 RuleLog / EntityLog 联合对齐 Governance。
