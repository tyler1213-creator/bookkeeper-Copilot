...
- rule promotion 发现侧尚未冻结：确定性发现 job 当前只服务 Rule Match / Rule 侧 promotion，具体阈值谓词、CaseLogEvidence 打包、审核 inbox 写入和 rule 升级固定执行路径归 RuleLog / RuleMatch 治理；本节点 runtime 不产 candidate、不维护统计。
...
- Rule 侧 promotion 发现 contract 落定：谓词归属、审核 inbox、CaseLogEvidence、固定执行路径和 Human Review 触发接缝。
