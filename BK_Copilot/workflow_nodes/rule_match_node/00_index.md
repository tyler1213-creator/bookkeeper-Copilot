...
- rule promotion 发现侧尚未冻结：确定性发现 job 当前只服务 Rule Match / Rule 侧 promotion，具体阈值判句、CaseLogEvidence 打包、审核 inbox 写入和 rule 升级固定执行路径归 RuleLog / RuleMatch 治理；本节点 runtime 不产 candidate、不维护统计。
...
- JE Generator 接口尚未冻结；treatment “judgment-free 完整”的判句依赖 JE Generator 需要什么，属于跨节点 seam。
