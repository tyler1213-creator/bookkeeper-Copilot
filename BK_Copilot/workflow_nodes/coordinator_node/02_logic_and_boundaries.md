...
- 不在运行期裁决身份冲突；身份冲突 / merge / split 归 Human Review（会计师人发起）/ Governance，以及 ER 运行期判句 / 确定性发现的确定性冲突处理，不归 Coordinator 消费。
...
- force_pending / promotion_lock / rule / profile / review-governance candidate 等长期记忆或治理风险候选。
- merge / split、force_pending / promotion_lock 等治理候选只可作为非身份状态的治理 / review candidate 交给对应圈外路径；Coordinator 不裁决、不落地。
...
- active rule、force_pending / promotion_lock、governance event、case authority、classification memory。
...
- 身份冲突、merge / split 的运行期裁决。
- 自行落地 Entity / Alias / Case / Rule / force_pending / promotion_lock / Transaction / Intervention durable 变更。
...
- 长期 authority 变化，例如 rule create / upgrade / relaxation、force_pending / promotion_lock 设置 / 解除、merge / split、entity lifecycle mutation 等。
- case-derived rule promotion、force_pending / promotion_lock candidate 转成 durable authority。
...
| governance / review candidate | accountant 答复暴露长期记忆、policy、rule、case-derived risk、profile 或身份治理风险。 | Governance / Review 等圈外路径 | 对应路径可评估是否需要 approval、review 或后续治理。 | 不代表 candidate 已持久化、已批准、已成为身份状态或 durable authority。 |
| Case Log 学习记录语义 | stable-linked finalized 交易经 accountant 确认后，可形成 `confirmed_by=accountant` 的可复用先例语义。 | Case Log 写入机制 / 统一 finalization mechanism（圈外机制，L4 / seam） | 后续可凭 finalization proof 评估写入资格。 | 不代表 Coordinator 直接写 Case Log，不适用于 unknown entity 交易，不代表 exact schema 已冻结。 |
...
如果出现身份冲突、merge / split 或身份风险：

- authority 顺序：Coordinator 不在运行期裁决身份冲突；accountant 明确 identity confirmation 可按身份确认路径推进，冲突发现与解决归治理 / Entity Log 相关路径。
- 本节点行为：不在运行期裁决身份冲突，不把身份风险投影成当前会计师问题；按已确认交互结果推进。
- 是否生成 review / governance candidate：身份冲突的 merge / split 解决由会计师人发起经 Human Review 处置、Entity Log 记录其变更语义等圈外路径处理；系统不自发做语义 merge / split 判断（原 `merge_split_candidate` 候选字段已删，见 Decisions D1）。
...
2. Pending 中 identity 背景、阻塞原因、已执行工作、可选推断性建议、evidence refs 的 exact 字段；身份风险不属于 Coordinator 所需字段。
...
9. force_pending / promotion_lock / rule / profile / review-governance candidate 的 exact 类型 / schema；消费方多为圈外。
...
4. Governance（授权确认 = 会计师 Human Review 签字 + Finalization 凭证强制；无独立 Governance Review Node）：长期 authority 变化、policy / rule / merge / split 等批准路径。
5. Profile / Structural Match、Knowledge Summary、Human Review Node、interaction_agent、Onboarding 的正式边界。
6. 身份冲突、merge / split 的发现与解决机制；归治理 / Entity Log 相关路径，不在 Coordinator 运行期裁决。
7. 极端身份安全 / 证据真实性情形的运行期处置；本轮不预设边缘情形。
8. ~~ER `candidate_signal` 是否最终也进入 Coordinator~~：已关闭（`candidate_signal` 已删除，见 Decisions D1）；本轮输入面仍锁为 CJ Pending。
