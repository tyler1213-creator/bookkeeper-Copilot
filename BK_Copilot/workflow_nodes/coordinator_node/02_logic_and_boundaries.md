...
- 不读取 identity risk flags；疑似同名、疑似 merge、identity conflict 等归 Human Review（会计师人发起）/ Governance，以及 ER 运行期判句 / 确定性发现的确定性冲突处理，不归 Coordinator 消费。
...
- 是否生成 review / governance candidate：身份冲突由 Entity Log §4 `merge_split_candidate`（生产者 = 确定性发现 / ER 运行期判句 + 会计师在 Human Review 人发起）等圈外路径处理；系统不自发做语义 merge / split 判断。
