...
Alias 的写入资格和审批语义已经冻结；写入执行者、确切时机 / 顺序和 finalization mechanism 尚未冻结。
...
- Entity Resolution 判断为 `new_stable_entity` 时，Entity Log publication 只确认 entity 本体；Alias 是否够格写入由本节资格规则决定。

| Writer | 可以写什么 | 写入类型 | 需要 approval 吗 |
| --- | --- | --- | --- |
| 待 L4 / seam 冻结的 memory write / finalization 执行机制 | 已够格的 raw transaction surface text -> stable entity identity reuse relationship | durable write；够格即自动落库；具体执行者、调用方式、写入时机 / 顺序未冻结 | 初始够格写入不需要额外 governance / accountant approval gate；correction / split 重判等治理性变更的审批路径未冻结 |

执行时机的非约束性 hint：Alias 写入倾向与创建 stable entity 同时发生，因为两者成立时机一致；该倾向需待 seam 阶段确认，不能作为当前标准。
...
- Alias 的写入执行者、确切时机 / 顺序和 finalization mechanism；倾向与创建 stable entity 同时写入，但需待 seam 阶段确认。
...
4. Alias 写入执行者、确切写入时机 / 顺序和统一 memory write / finalization 机制；写入资格和“无需额外审批”已冻结。
