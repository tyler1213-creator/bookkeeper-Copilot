...
| reusable permission（复用权限） | `Case Log` 中的 `use_level` 语义（exact enum 留 M3） | 限定每条 case 可作为正面先例、仅例外、反面 / 冲突、不可升级 rule，或已失效 / superseded | rule approval、governance approval、Entity Log force_pending / promotion_lock |
| rule promotion / force_pending / promotion_lock review evidence（规则升级 / 控制状态审核依据） | Case Log case history | 可以作为治理流程判断 rule promotion、force_pending / promotion_lock 的历史依据 | 直接 mutation、approved rule、放宽 / 解除后的 force_pending / promotion_lock、Entity Log authority、Case Log 存储字段形式的 candidate queue |
...
| Rule promotion / force_pending / promotion_lock review flow（规则升级 / 控制状态审核流程，具体节点未冻结） | 读取 entity-linked case history，评估 rule promotion、force_pending / promotion_lock | case pattern、exception / correction history、`use_level`、case history distribution 派生视图 | 只能作为治理依据；不能直接修改 Entity Log、Rule Log、Governance Log 或 force_pending / promotion_lock |
...
| Governance（授权确认 = Human Review 签字 + Finalization 凭证；无独立 Governance Review Node） | 评估 rule change、force_pending / promotion_lock change | case evidence、supporting refs、读取层派生聚合 | 必须通过 accountant approval（Human Review 签字）；Case Log 只提供依据 |
...
| Rule promotion / force_pending / promotion_lock review flow（未冻结） | 读取 Case Log 生成治理候选 | temporary handoff / candidate outside Case Log storage | 是；不能直接把 candidate 变成 Entity Log / Rule Log / Governance Log mutation；transient 候选 handoff 不作为 Case Log 存储字段（原拟 candidate_signal_refs 不设；ER `candidate_signal` 已删，见 Decisions D1） |
...
- force_pending / promotion_lock review candidate（控制状态审核候选）。
...
- 作为 Rule Log / Entity Log / force_pending / promotion_lock mutation：必须经过治理 / review 路径，并获得 accountant / governance approval；具体路径未完全冻结。
...
```text
Case Log evidence
-> rule promotion / force_pending / promotion_lock review candidate generated outside Case Log storage
...
| Transaction Log（交易日志） | Transaction Log 保存 final 交易事实、完整 processing path、修改历史与 audit trail，是审计 source of truth。Case Log 只引用 transaction_log_ref / finalization proof，只保存最新可学习状态，不存修改历史、不存完整 processing path、不替代审计记录。entity 未解析即完成分类的交易只 finalize 到 Transaction Log，不进入 Case Log | 身份缺口标记落点已定为 Transaction Log（Transaction Log L2 用户拍板锁定；字段形态留 L3）。仍未冻结：Case Log 与 Transaction Log 的 exact trigger order、writer、多 log finalization、correction / reversal / split 后的执行机制 |
| Entity Log（实体日志） | Entity Log 保存 stable entity identity、Alias、status、force_pending / promotion_lock；Case Log 按 entity_id 组织案例，但不修改 entity authority。merge / split / archive 后旧 case 归属由治理层裁定，Case Log 不裁判但服从结果 | case-derived risk 进入 Entity Log 的治理路径、merge / split / archive 后重新归属执行机制未冻结 |
| Alias Log（别名日志） | Alias 的 authority 在 Entity Log；Alias Log 是 alias→entity 反查 projection / index，不持有 authority。Case Log 可保存本笔交易自身 raw surface text 的 alias 表面快照 / 引用，用于模式区分，由写入路径 finalize 时携带。Case Log 对 Alias Log 零读取权限；该 alias 不通过查询 / 匹配 Alias Log 得到，不构成第二个 Alias Log reader，也不持有、不立 Alias authority | 基于交易 raw surface text 快照的 case retrieval contract 未冻结；Alias correction 如何影响 Case Log alias 快照属于后续机制 |
...
| Governance Log（治理日志） | Governance Log 保存高权限变化、批准、拒绝、降级、合并和拆分历史；Case Log 衍生的 force_pending / promotion_lock / rule 相关变化必须经过治理或 accountant approval 才能 mutation | Case Log evidence 与 Governance Log event / audit trace 的 exact contract 未冻结 |
...
- authority 顺序：Entity Log 和已生效 Governance Log 对 entity identity、status、force_pending / promotion_lock 优先。
...
- 是否阻断自动化：如果没有 approved rule 或上游 eligibility 放行，不得把 repeated case 当 rule 自动放行。
...
9. case-derived rule promotion、force_pending / promotion_lock candidate 的 exact governance contract。
