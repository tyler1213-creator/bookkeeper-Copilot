...
- case history pattern：只能作为 case-based judgment 或 rule / automation / entity risk review 的历史依据，不能直接成为 deterministic rule 或 durable mutation。
...
- rule / automation / entity risk review 所需的历史依据；候选生成与审批归治理路径，候选信号本身不作为 Case Log 存储字段。
