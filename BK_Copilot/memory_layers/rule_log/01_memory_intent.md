...
- **稳定唯一 `rule_id`**：每条 rule 一个、贯穿其生命周期不变（改动不换 id），作 rule 的引用 / 审计 handle——RuleMatch 命中直接输出 `rule_id`、Transaction Log rule-hit 引用也用它（裸 id、不钉版本，见 Decisions D9）；exact 格式留 M3。
- `entity_id` 主索引。
...
- EntityLog force_pending / promotion_lock 本体。
- raw evidence blob。
- LLM reasoning。
- 相似描述。
- 模糊自然语言规则。
...
- Entity Log：stable entity identity、entity lifecycle、force_pending / promotion_lock 等 control state 的主档案。
...
- 支持自动化率提升：Rule Log 让系统在 stable entity 与上级 eligibility 已放行（force_pending 未命中）时，不重新判断会计处理，直接复用已批准的 deterministic rule。
...
- Rule Log 中存在 executable rule 只是 RuleMatch 自动化的必要非充分条件；EntityLog force_pending（经上游 eligibility router 改道）仍是上级放行控制。
- 每条 rule 有稳定唯一 `rule_id`，贯穿生命周期不变；Rule Log 只存当前可执行状态，**rule 变更历史归 Governance Log**（不在 Rule Log 内做版本化，避免与治理裁定竞争）。审计某笔历史交易"当时那条 rule 是什么" = `rule_id` + Transaction Log 交易时间 + 回放 Governance Log 变更事件（见 Decisions D9）。
