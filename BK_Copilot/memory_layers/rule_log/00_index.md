...
- **L2·外阻，Governance / review path**：rule promotion 发现侧（确定性发现 job 只套用 Rule 侧判据）、rule candidate queue / 审核 inbox、system-proposed promotion request、CaseLogEvidence 打包格式、accountant approval workflow。
...
- automation_policy exact enum 以及哪种取值表示放行 RuleMatch 未由 EntityLog / RuleLog / Governance 联合冻结前，不进入 M3 data contract。
- rule mutation path、writer exact contract、reader query contract、projection / cache / permission 边界未冻结前，不进入 M3 / M4。
- promotion 发现侧判据、审核 inbox、固定执行路径、CaseLogEvidence 打包格式、accountant approval workflow、accountant direct write 机制、merge / split 后 rule 重判机制未由 Rule 侧 / Governance / L4 seam 收口前，不进入实现。
