...
- **L2·外阻，Governance / review path**：rule promotion 发现侧（确定性发现 job 只套用 Rule 侧谓词）、rule candidate queue / 审核 inbox、system-proposed promotion request、CaseLogEvidence 打包格式、accountant approval workflow。
...
- promotion 发现侧谓词、审核 inbox、固定执行路径、CaseLogEvidence 打包格式、accountant approval workflow、accountant direct write 机制、merge / split 后 rule 重判机制未由 Rule 侧 / Governance / L4 seam 收口前，不进入实现。
