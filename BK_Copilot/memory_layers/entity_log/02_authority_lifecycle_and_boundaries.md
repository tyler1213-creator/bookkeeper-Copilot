...
| --- | --- | --- | --- |
| Onboarding Node（初始化节点） | sufficiently supported initial entity foundation（证据充分的初始实体基础） | direct / candidate，具体边界未完全冻结 | 其他候选需后续 approval（批准） |
...
- merge / split（合并 / 拆分）后 aliases（别名）、rules（规则）和 cases（案例）的迁移规则。
