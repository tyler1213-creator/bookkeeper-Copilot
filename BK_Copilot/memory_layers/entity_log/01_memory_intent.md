...
- entity-level control state（实体级控制状态）：系统无法在 runtime 就地、确定性读取 automation_policy 或身份级风险卡点。
...
- 控制状态（entity-level control state）：entity 级 automation_policy，以及只会在 runtime 当卡点用的身份级 risk flags，例如疑似同名、疑似应 merge、identity conflict。会计师表达“以后遇到这个 entity 要当心 / 系统老判错”时，不新增自由文本 note 字段；应落成 automation_policy 降级、身份级 risk flag 和 rationale ref。
...
- automation_policy 当前控制状态。
- 会作为 runtime 卡点的身份级 risk flags。
...
- classification history、“这个 entity 通常记成哪个 COA”、repeated outcome pattern：归 Case Log；分类不稳定、记账结果漂移等 case-derived risk 不写入 Entity Log risk_flags。
...
- 让下游通过稳定 entity identity、lifecycle / supersession 和身份级 risk flags 避免误用已合并、归档或身份冲突的 entity。
- 让 automation_policy 把 entity-level control state 转成明确、可审计、会计师可控制的自动化边界。
...
- risk_flags 只保留会作为 runtime 卡点的身份风险；分类不稳定、记账老飘等 case 衍生风险不进入 Entity Log。
