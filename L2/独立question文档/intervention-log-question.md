...
与 `interaction_agent_question.md` 同级存放（均为"讨论结论入口"类文档，非正式 spec）；原 `Coordinator Question.md` 已删除，Coordinator 主题已落正式草案。
...
- 要把交互转成 durable 记忆 / 规则 / policy，仍须经治理 / review / case memory 等正规路径（原 `Coordinator Question.md`，已删除 / 已归档，内容已吸收进 `coordinator_node/` 正式草案）。
