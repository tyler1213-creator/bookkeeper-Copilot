...
「批次 = 单张 Bank Statement」的定义，以及一次提交多张表如何排序（串行 vs 并行快照），属 Coordinator / 编排层与会计师交互逻辑，CJ L2 引用而不冻结（记于 `Coordinator Question.md`）。
...
- 「批次 = 单张 Bank Statement」的定义、一次提交多张表的排序（串行 / 并行快照），以及由此产生的跨运行重复提问，属 Coordinator / 编排层与会计师交互逻辑，记于 `Coordinator Question.md`，不在 CJ 冻结。
