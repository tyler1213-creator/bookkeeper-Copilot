> ⚠️ **已过时 / superseded（owner 2026-06-27）** — 本 L2 提案仅作历史来源保留。CJ 设计的当前权威 = `BK_Copilot/workflow_nodes/case_judgment_node/` 正式草案 + 即将产出的 CJ L3 schema；**勿据本文件回灌已删除的概念 / 字段**。

# Case Judgment L2 提案

> 进行中。本提案逐主题收口；当前已讨论确认 Q1（输入加载分层）、Q2（无先例时的自动判断）、Q3（Entity Resolution 输出对本节点的身份约束）、Q4（输出状态分类法）、Q5（自动进 Pending 的触发）、Q6（JE Generator 定位）、Q7（Pending handoff 最小语义契约）、Q8（candidate signals 范围）、Q9（High Confidence 后的路由）、Q10（Reasoning / Audit Trace 形态）、Q11（mixed-use 等 soft risk 处理）、Q12（多源冲突优先级与保守原则）、Q13（external lookup 边界）、O1（代码与模型分工、执行顺序与路由机制）、O2（Pending 承接与 reason 定位）、G1（Case Log 先例读取：代码递客观事实、CJ 当辅助证据权衡）、G3（批次内记忆一致性：身份即时、案例不回写）、2.18（兜底定位与 runtime-only 边界）。
>
...
- **上游路由 / 入口（拓扑）**：分流权属 Entity Resolution。ER 认出 stable entity 后，按该 entity 名下是否存在 active rule 分流：有 active rule → 路由到 Rule Match 判定命中哪条；无任何 active rule → 由 ER 直接路由到 Case Judgment。因此 CJ 的入口是「没有可执行 active rule 的交易」，共三类：(1) ER 对『entity 无 active rule』者直接路由来的交易；(2) Rule Match 对『entity 有 rule 但本笔无命中』输出的 `rule_miss`（`BK_Copilot/workflow_nodes/rule_match_node/01_functional_intent.md`）；(3) ER 的 `unknown`。该路由由 ER 自身还是 workflow router 执行的精确分工属 L4 / seam。
- **上游证据（已锁）**：Evidence Intake / Preprocessing 提供可追溯 evidence 与 evidence refs，并分配稳定 `transaction_id`。
- **上游结构（圈外·待审计，未锁；以下为 CJ 侧假设）**：Profile / Structural Match 已确认当前交易未被结构性路径完成；identity-independent 的结构性交易（如 internal transfer、bank fee、interest）主要由此路径处理。该节点尚无 `BK_Copilot/` 正式草案，待其正式审计确认；本提案只挂起引用，不当冻结合同（见缺口地图 Case Judgment section）。
...
- **entity-level Knowledge Summary（圈外·待审计，未锁，下游记忆层；CJ 侧假设）**：按 `entity_id` 锁定后按需注入 Case Judgment，自带 authority / usage 护栏（读取方向见 `BK_Copilot/memory_layers/case_log/01_memory_intent.md`）。Knowledge Summary 尚无正式草案，待其正式审计确认；本提案只挂起引用（见缺口地图）。
- **JE Generation Node（圈外·待审计，未锁，下游；CJ 侧假设）**：CJ 假设其为纯确定性构造层，消费本节点 High Confidence Classification 的确定 (COA, HST/GST) 决定、不做分类判断。该节点尚无正式草案，待其正式审计确认此契约；本提案只挂起引用（见缺口地图）。
...
High Confidence Classification 输出后、调用 JE Generation Node 之前，须经一段确定性代码校验：将 CJ 输出的 COA 科目与该客户 COA 全集匹配——命中则放行进入 JE 构造；未命中则**反馈回 CJ 重判**：明确告知 CJ「所选 COA 不在客户 COA 全集内」，由 CJ 据此重新分类。该重判是由代码确定性条件（校验失败）触发的受控再调用（符合 2.14「多次模型调用由代码编排」，非模型自决）。**安全阀**：若重判后仍无法落到合法 COA，则转 Pending 交会计师，保证不产生无归处孤儿（守 2.18）。该校验是纯确定性代码、不做会计判断，可作为 CJ 节点末尾的独立检查器（不必由 CJ 模型执行），亦可直接并入 JE Generation Node（任何节点送来的交易在构造分录前本就都需校验科目是否在 COA 全集内）；两种落点皆可，本 L2 不强制其一。重判次数上限、反馈 payload 形态与详细纠错 UX 属 L4，MVP 阶段不细化（见缺口地图）。

**为什么（锚定产品目标）：**
审计性、自动化率、单一职责。会计分类的权威集中在 Case Judgment，分录构造与校验集中在 JE Generation Node，避免分类逻辑在两处重复或漂移。沿用旧系统 build_je_lines「纯确定性计算、单一职责、不做分类判断」的边界。COA 匹配以确定性代码而非模型完成，保证落账科目必属客户 COA 全集。
...
**浮现的新 open boundary：**
COA 校验的落点（CJ 末尾独立检查器 vs 并入 JE Generation Node）两可、本 L2 不强制其一；校验失败的 L2 行为已定（反馈回 CJ 重判 → 仍失败转 Pending），其重判次数上限、反馈 payload 形态与纠错 UX 的 exact contract 属 L4，MVP 阶段不细化，留缺口地图。
...
**结论：**
判定为 High Confidence Classification 后，直接调用 JE Generation Node 生成分录，不在生成前设置强制人工审核门。交易 finalize 后，结果在 Case Log 与 Transaction Log 留痕、并标注为「AI 高置信度自动完成」；CJ 本身不直接执行写入，只发出 case memory update 信号（见 2.8），由专门的 finalization / 记忆写入机制按各自资格落库（Case Log 写入需 stable-linked + finalized + `transaction_log_ref`）。

事后人工如何查看高置信度结果（如按置信度分栏的当日汇总界面、抽查机制）属 UI / 后续设计，不在本 L2 冻结。

**为什么（锚定产品目标）：**
自动化率、审计性。若每笔高置信度交易都强制人工介入，自动化的核心价值即丧失。直接进 JE 保住自动化率；finalize 后 Case Log + Transaction Log 留痕并标注来源，保证高置信度结果事后可审、可抽查、可定位，不牺牲审计性与 accountant control。沿用旧系统「高置信度直接构造 JE、并在报告中标成 AI 高置信度一档供事后审」的边界。

**拟改：**

- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:下游契约 / 路由` → `High Confidence Classification 直接进入 JE Generation Node 构造分录，不设生成前强制人工审核门。交易 finalize 后结果在 Case Log 与 Transaction Log 留痕并标注为 AI 高置信度自动完成；CJ 不直接写入，只发出 case memory update 信号，由 finalization / 记忆写入机制按各自写入资格落库。事后人工查看的呈现与抽查机制属 UI / 后续设计。`

**排除的替代 + 理由：**
High Confidence 也先挂一道 review 门才进 JE。理由是这会让每笔交易都需人工介入，抹掉自动化核心价值；「hard block 一律不自动放行」已由两类输出模型（2.4）承载，剩余的高置信度结果以「直接进 JE + finalize 后双 log 留痕标注 + 事后抽查」即可兼顾稳健与效率。
...
### 2.11 Mixed-use / 个人消费等 soft risk：默认公司支出，证据明确指向非公司才上交
...
- **唯一的停下条件**：仅当手上证据已**明确**指向「这不是公司支出」时，Case Judgment 才不打 High Confidence，把它作为一次决策交回会计师（个人 vs 公司 / 费用 vs Shareholder Loan / Owner's Draw 的处理选择）。中间「看着像公司、可能夹带私人、但分不清」的一大片，一律默认公司支出放行。证据明确指向私人即交回，不因金额小就替会计师把它默认费用化为公司支出。
...
- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:决策权限 / soft risk 处理` → `对 mixed-use / 个人消费风险，Case Judgment 默认按公司支出正常分类放行，不充当稽查员、不主动拆解证据搜寻私人物品；小票品类等证据仍须用于确定分类。仅当手上证据明确指向非公司支出时，不打 High Confidence，作为一次个人 vs 公司的决策交回会计师（不因金额小就默认费用化）。其余「可能夹带私人但分不清」的交易默认公司支出放行，靠事后报告复核（见 2.9）兜底。mixed-use 属会计师立场判断而非证据缺口，不以补证据 Pending 套用。`
...
**浮现的新 open boundary：**
会计师 / 客户既定处理立场如何沉淀与读取（Profile 结构事实、entity-level Knowledge Summary、Case Log 先例）的 exact 机制留对应记忆层与 seam。
...
- **第 0 层 · 硬权威阻断（最高，证据再强不可越过）**：身份硬阻断（ER `unknown`）；Entity Log `automation_policy` 收紧值（review_required / disabled）与身份级 risk flags；Case Log `use_level` 标记必审 / 不可复用 / 已失效。命中即封顶，结果为 Pending，不进 High Confidence。依据：这一层都是会计师 / 治理决定**已经落地到 CJ 读得到的标记上**的结果（automation_policy、risk flags、use_level、身份状态），可能基于系统看不到的额外上下文；CJ 据这些标记裁定，**不另行读取任何治理或规则的原始账本**。
- **第 1 层 · 权威客户结构事实（Profile）+ 限定范围的会计师确认**：Profile 结构事实（business_type、has_hst_registration、province、tax_config、bank_accounts）对其所管维度有决定权（如 has_hst_registration=false → HST exempt，压过小票税额；business_type → Shareholder Loan vs Owner's Draw）；以及 Case Log 中 `confirmed_by = accountant` 的、针对本 entity / case 的明确确认或纠正，仅在其确认范围内有效，模糊批注不泛化。
...
- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:冲突处理 / 决策权限` → `多源冲突按统一优先级裁定（高→低）：(0) 硬权威阻断（ER unknown、Entity Log automation_policy 收紧值与身份级 risk flags、Case Log use_level 必审 / 不可复用 / 失效）——均为会计师 / 治理决定已落地到 CJ 读得到的标记，命中即封顶为 Pending，CJ 不另读治理 / 规则原始账本；(1) 权威客户结构事实 Profile + Case Log 中 confirmed_by=accountant 的限定范围会计师确认；(2) 当前交易证据；(3) Case Log 先例；(4) 通用知识 + Knowledge Summary。保守原则：歧义不猜、冲突不挑顺眼版本，无法在该序内干净解决即输出 Pending 并讲清冲突点，绝不硬编 winner。`
...
**为什么（锚定产品目标）：**
自动化率、有证据支持的建议、审计性。大量零售 / 冷门商家的 description 不足以确定品类，禁止 CJ 联网会丧失有效信息源、压低自动化率；套用 ER 正式文档已锁的"行为非权威、只认可追溯来源、不达唯一即 Pending、保留可追溯引用"纪律，使联网在提升覆盖率的同时不放大 SEO / 同名 / 结果漂移风险，并守住身份与分类的 seam。
...
**结论：**
Case Judgment 是代码驱动的流程，大模型是其中被调用做判断的工具（它在自己的判断回合内可自行联网取证，见 2.13），但不是一个自主编排流水线、自持路由权的 agent。分工与顺序固定为「代码 → 模型 → 代码」：
...
- **模型（在代码备好的上下文内做判断）**：识别交易主体 / 性质、选择 COA、确定 HST/GST、判断当前证据是否足以落到唯一站得住的 (COA, HST/GST)、生成 reasoning，并给出它对自身判断的置信度。模型**可在判断过程中自行决定发起联网搜索**以补充会计事实证据（用途与采信边界见 2.13）。模型不持有控制流 / 路由权、不决定流水线下一步走哪个节点；联网搜索是其判断职责内允许的取证工具，不等于把流水线控制权交给模型。
...
- 两者相互独立。最终是 High Confidence 自动落地还是 Pending，一律由代码裁定：代码先读取所有确定性字段，**只要存在硬阻断（如身份 `unknown`、Entity Log `automation_policy` 收紧值与身份级 risk flags、Case Log `use_level` 标记必审 / 不可复用），无论模型置信度多高，一律 Pending**；不存在硬阻断时，代码才采信模型的置信度作为放行信号（足够确信 → 自动落地；否则 Pending）。
...
硬阻断的判定必须基于结构化字段（enum / flag）由代码确定性计算；任何需要读自由文本、做语义解读才能发现的约束都不算硬阻断，必须降级为 soft risk，交模型在限定框内判断。这些硬阻断的**语义级清单已有**（见 2.5 / 2.12：身份 unknown、Entity Log automation_policy 收紧、Case Log use_level 必审 / 不可复用 / 失效、身份级 risk flags），**但尚未结构化为字段**——它们代表上游 / 记忆层中会影响 CJ 判断的功能，exact 字段名 / enum 尚未定下；「硬阻断 = 代码可算的结构化字段」是这些功能结构化后必须满足的硬标准。

**模型调用与工具使用：**
模型的一次判断是一个逻辑回合；回合内允许它自行发起联网搜索取证（见 2.13），这不算「多次独立调用」。需区分三件事：(a) 模型在判断回合内自取会计事实证据——允许，由模型自决；(b) 由代码按确定性条件重新发起一次模型判断（如 COA 校验失败后的重判，见 2.6）——允许，但触发权在代码、不由模型自决；(c) 模型自主决定流水线控制流 / 路由 / 是否让整条管道再跑一遍——禁止。此约束守住「模型不持路由与控制流权、不是跨节点自主 agent」，但不限制它在判断回合内联网取证。

**为什么（锚定产品目标）：**
审计性、accountant control、自动化率。把放行许可交给代码的确定性字段判定，使「系统不得自动落地」成为硬保证，不依赖模型是否听话，杜绝长上下文淹没关键约束的失败模式；同时保留模型对被阻断交易的判断与建议，最大化会计师单次即可解锁的比例，不牺牲自动化与体验。把置信度与许可分离，避免用路由规则逼模型谎报置信度、或损坏给会计师的建议。「分类器不是 Agent、LLM 只在代码备好的上下文里做一次判断、硬阻断由代码处理」这一边界，按上述产品目标重新论证后采纳（旧系统曾如此表述，但此处不以旧系统为正确性依据），并去掉其按 (pattern, direction) 索引与 risk-pack 的具体实现。

**拟改：**

- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:决策权限 / 代码与模型分工` → `Case Judgment 是代码驱动流程，模型是被调用做判断的工具（判断回合内可自行联网取证），不是自主编排流水线的 agent。顺序：代码（触发判定 + 上下文组装 + 读所有确定性字段算出本笔最多能到哪一步 + 备好结构化约束）→ 模型（框内一回合判断：主体 / 性质、COA、HST、证据充分性、reasoning、自评置信度；可自行发起联网搜索取证见 2.13，但不持路由 / 控制流权、不决定流水线下一步）→ 代码（裁定路由 + 校验 + 转交 / 写入）。置信度与许可分离：模型答"是什么 + 多确定"，代码答"系统能否自动落地"。最终路由由代码裁定：存在硬阻断则无论置信度多高一律 Pending；无硬阻断才采信模型置信度放行。模型不持路由权。被阻断的交易仍可让模型给建议附于 Pending；告知模型"将交人工"只为塑造输出形态，不为压置信度。硬阻断必须由结构化字段确定性可算，需语义解读的降级为 soft risk。模型判断回合内可自行联网取证；由代码按确定性条件发起的重判（如 COA 校验失败）触发权在代码；模型不自主编排流水线控制流 / 路由。`
...
- 把 Case Judgment 实现为模型自主**编排流水线控制流**的 agent loop（由模型决定路由、是否让整条管道再跑、下一步走哪个节点）。理由：会把控制流 / 路由决策权交回模型，违反「模型不持路由与控制流权」。注：模型在判断回合内自行联网取证（2.13）不属此列——那是受限的取证工具使用，不是对流水线的自主编排。

**浮现的新 open boundary：**

- 各上游 / 记忆层用于硬阻断判定的结构化字段（enum / flag）尚未定形（语义级清单已有，见 2.5 / 2.12）；其 exact 字段名 / enum 待这些功能结构化后于 L3 冻结。
...
4. 会计师要求对 entity 做特殊处理（如「以后不可高置信度分类」）→ Coordinator 收到信号后产出 candidate signal。该 candidate 的消费者与落更机制属 Coordinator / 治理侧设计，不在 CJ 这里展开。
...
- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:Pending 行为 / 下游契约` → `Case Judgment 的 Pending 唯一 consumer 是 Coordinator，不做多路分流。reason 只解释模型为何输出该 Pending（为何不确定是哪个分类 / 为何无法判断），是 2.10 论证链在 Pending 上的特例，用于定位判断断点；reason 不开补救药方、不负责把问题结构化成会计师提问（属 Coordinator）。模型语义判断有价值候选时，候选由模型随 Pending 产出，Coordinator 负责组织成可读提问（选项 1/2/3 / Other），不重做分析。下游：Coordinator 向会计师提问 → 确认且信息足够则转 JE 格式调 JE Generation Node；不够则追问；会计师要求 entity 特殊处理则 Coordinator 产出 candidate signal（其消费者与落更机制属 Coordinator / 治理侧设计，不在 CJ 展开）。`
...
### 2.17 批次内记忆一致性：身份即时传播、案例不回写

**结论：**
CJ 在同一批次内对 durable memory 的读取遵循「身份即时、案例不即时」的分层一致性。

- **案例记忆（Case Log）批内不回写**：CJ 对 Case Log 先例读取的是批起始固定快照；同一批次内 CJ 各次运行互不学习，本批次产生的判断不在批内回写、不影响本批后续交易。（沿 Profile 批起始快照原则，且案例写入本就发生在 finalization 之后，见 2.8。）
- **身份记忆（Entity Log / Alias）批内即时可见**：批内由 Entity Resolution 新建的 stable entity 必须在进入下游前同步 publish，对后续同批交易的 identity consumer 即时可见（沿用 `entity_resolution_node/02` §4）。CJ 据此可把它当身份基础。即同一新对象在一张表内重复出现，各次共享同一 stable entity_id，但各次互不构成对方的 case 先例。
- **跨批次学习只经 finalize 写入发生**：上一批次的判断要影响下一批次，必须先经 finalization（含会计师确认、记忆写入）落地；未 finalize 的判断不跨批传播。
- 与 2.16 衔接：批内新建 entity 的各次出现，rollup 递给 CJ 的先例事实均为「历史 0 次」，每次都凭当前证据独立判断。

「批次 = 单张 Bank Statement」的定义，以及一次提交多张表如何排序（串行 vs 并行快照），属 Coordinator / 编排层与会计师交互逻辑，CJ L2 引用而不冻结（原记于 `Coordinator Question.md`，已删除 / 已归档，内容已吸收进 `coordinator_node/` 正式草案与 Coordinator L2 提案）。

**为什么（锚定产品目标）：**
审计性、自动化率、accountant control。身份批内即时传播防止同一新对象在一张表内被建成多个 entity，保住身份一致性；案例批内不回写防止人工确认前的判断自我学习 / 滚雪球，也避免系统卡在等会计师回复（自动化率）；两者方向相反，但分别服务「同一对象身份要稳」与「分类不可未审先学」。跨批次只经 finalize 学习，保证学习信号都来自已落地、可追溯的记忆，守住「Case Log 写入需 finalization proof」的已锁边界。

**拟改：**

- `BK_Copilot/workflow_nodes/case_judgment_node/02_logic_and_boundaries.md:读取对象 / 批次一致性` → `CJ 在同一批次内：对 Case Log 案例先例读批起始固定快照，批内各次运行互不学习、本批判断不批内回写；批内由 Entity Resolution 新建的 stable entity 即时可见（entity_resolution_node/02 §4），CJ 可作身份基础——身份即时、案例不即时。跨批次学习只经 finalize 写入发生。批次定义与多表排序属编排层，不在本节点冻结。`

**排除的替代 + 理由：**
让 CJ 在批内对案例记忆即时学习（同一对象第 2、3 次复用第 1 次判断）。理由：人工确认前的判断自我累积会滚雪球放大错误，且要求批内串行 finalize 会把系统卡在等会计师回复，削弱自动化率；身份一致性已由 ER 的批内 publish 单独保证，无需让案例也即时回写。

**浮现的新 open boundary：**

- 「批次 = 单张 Bank Statement」的定义、一次提交多张表的排序（串行 / 并行快照），以及由此产生的跨运行重复提问，属 Coordinator / 编排层与会计师交互逻辑，原记于 `Coordinator Question.md`（已删除 / 已归档，内容已吸收进 `coordinator_node/` 正式草案与 Coordinator L2 提案），不在 CJ 冻结。

### 2.18 CJ 的兜底定位与运行边界

**结论：**

- **兜底定位与不可删除性（恒有归处、不拒收）**：CJ 是整个系统的兜底与沟通桥梁，也是实现高自动化的核心环节。与 ER 高度依赖 Evidence Intake 交付的标准化输入、必须靠这些输入识别 Entity 不同，CJ 的存在正是为了兜住结构化 / 规则化路径处理不了的一切情况——它永远能把交易产出为可交付结果：要么 High Confidence Classification（直接会计分类），要么 Pending（交会计师确认）。正因如此 CJ 不可被删除、合并或内联：删掉它，系统就失去「任何能流通到这里的交易都有归处」的灵活兜底，自动化率与「凡机器拿不准都干净回到人」的保证同时崩塌。CJ 不「拒收」交易、不产生无归处的孤儿；其输出恒为 2.4 的两类之一。
- **触发不设 ER 式硬前置门（保持灵活）**：系统没有权利拒绝任何交易的流通。任何一笔能在系统中流通并到达 CJ 的交易，CJ 都应处理，不设 ER 那种「缺某结构化前置即拒绝受理」的硬触发门。成文时此处保持灵活，以「让实现 Agent 理解兜底逻辑、写出合理代码」为准，不生搬模板的触发条件小节。（结构性无效 / 损坏输入的极端兜底见下条 DEFERRED。）
- **runtime-only**：CJ 是 runtime 的逐笔节点。它不参与 onboarding / 历史回填学习（归 onboarding / knowledge compilation）；经 Pending 由会计师解决的交易不重新进入 CJ（分类在 Coordinator 路径完成，沿 Entity Resolution / Coordinator 已锁结论）。
- **无效 / 损坏输入与错误纠错机制暂不设计**：CJ 对结构性无效 / 损坏输入（如缺 transaction_id、缺 ER 身份结果、缺 authority context）是否拒绝、被 CJ 与上游均无解交易的最终归处、谁发出纠错 candidate signal、纠错流程如何编排——均未设计，MVP 阶段不写入，DEFERRED 至缺口地图，待实战检验后再定。

**为什么（锚定产品目标）：**
accountant control、自动化率。把 CJ 钉成「恒有归处」的兜底桥梁，保证没有交易在 CJ 处被甩成无人负责的孤儿，凡机器拿不准的都干净地回到人；runtime-only 划清它与 onboarding / 知识编译的边界，避免职责蔓延。无效输入与纠错机制在下游纠错路径尚未设计时强行规定，反而会制造错误的拒绝路径、产出无归处交易，故 MVP 阶段显式不锁。

**拟改：**

- `BK_Copilot/workflow_nodes/case_judgment_node/01_functional_intent.md:定位 / 排除范围` 与 `02_logic_and_boundaries.md:边界` → `CJ 是系统兜底桥梁与高自动化核心，恒把交易产出为可交付结果（High Confidence 或 Pending），不拒收交易、不产生无归处孤儿；因此不可删除 / 合并 / 内联。系统无权拒绝交易流通：任何能流通到 CJ 的交易都应处理，CJ 不设 ER 式硬前置触发门，文档此处保持灵活、不列死触发条件。CJ 为 runtime 逐笔节点，不参与 onboarding / 历史回填；pending 经会计师解决的交易不重入 CJ。结构性无效输入的拒绝与否、无解交易最终归处、错误纠错机制 MVP 阶段不设计，见缺口地图。`

**浮现的新 open boundary：**

- CJ 结构性无效 / 损坏输入的响应、被 CJ 与上游均无解交易的最终归处、纠错 candidate signal 的发出者与纠错流程 → DEFERRED（未设计，实战检验后再定，记于缺口地图）。
...
- **本对象 L2 可否判定完成**：当前 Q1–Q13、O1、O2 收口（输入加载分层、无先例自动判断、身份约束、两类输出、自动进 Pending 触发、JE Generator 定位、Pending handoff 语义契约、candidate signals 范围、High Confidence 后的路由、Reasoning / Audit Trace 形态、mixed-use 等 soft risk 处理、多源冲突优先级、external lookup 边界、代码与模型分工及路由机制、Pending 承接与 reason 定位、Case Log 先例读取与权衡、批次内记忆一致性、兜底定位与 runtime-only 边界）。是否已覆盖本节点全部 L2 主题，见下方「L2 覆盖度复检」。
- **能否进 Stage 3**：L2 主题已实质收口（无仍需讨论的 L2 决策，O3 已按兜底定位 DEFERRED）。剩余为成文动作（C1 / C2 / C3 按模板汇入正式文档）与字段级 L3 schema；二者完成即可进 Stage 3。

### L2 覆盖度复检（对照模板 22 项 Stage 1-2 清单 + 第一性原理）

已覆盖：代码 / 模型分工与执行顺序(10/11, O1)、上游依赖(4)、下游影响(5)、读取对象(6, 含 Q1/Q13/G1)、直接写什么(7, =无)、candidate(8)、绝不能写(9)、证据不足(14, Q2/Q5)、Case Log 先例读取与权衡(14/16, G1)、批次内记忆一致性(6/20, G3)、兜底定位与 runtime-only 边界(1/2/3, 2.18)、歧义(15, Q12)、source conflict(16, Q12)、trace 保留(17, Q10)、trace 不成 authority(18, Q10)、对外契约面与 consumer(19, Q4/Q6/Q7/Q8/Q9/O2)、运行/记忆 seam(20, Q8)、未冻结项(21)、能否进 Stage 3(22)。

**仍开放、需讨论的 L2 主题：无。**

- **O3（触发 + 无效 / 被阻断 handoff 行为）已处理**：CJ 不设 ER 式硬前置触发门——系统无权拒绝交易流通，任何能流通到 CJ 的交易都应处理（见 2.18），故无需在文档里列死触发条件，成文保持灵活。CJ 定位为系统兜底桥梁，恒输出 High Confidence 或 Pending、不拒收交易（见 2.18）；结构性无效 / 损坏输入是否拒绝、被 CJ 与上游均无解交易的最终归处、纠错 candidate signal 发出者与纠错流程，均未设计，MVP 阶段不写入，DEFERRED 至缺口地图，待实战检验后再定。
...
- **C2 · Stage 1 Functional Intent（清单 1/2/3）**：为什么必须存在、为什么不可删除/合并/内联、唯一核心职责、明确排除范围、对产品目标的贡献——核心结论已在 2.18 收口（CJ 是高自动化核心 + 灵活兜底桥梁，恒把交易产出为可交付结果，删 / 并 / 内联即失去全系统的兜底归处保证），待按 Stage 1 模板成文。
