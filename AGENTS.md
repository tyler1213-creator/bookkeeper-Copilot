...
它不负责项目背景、稳定审计原则、当前任务状态、阶段规划、系统上下文摘要或具体主题的已确认结论记录。
...
- `TASK_STATE.md`：现在做到哪、下一步做什么、当前风险是什么。
- `AUDIT_CHARTER.md`：这个审计为什么存在、长期目标和稳定审计原则是什么。
- `PROJECT_BRIEF.md`：这个项目大概是什么业务背景。
- `PLANS.md`：当前审计处于哪个阶段、阶段目标是什么。
- `SYSTEM_CONTEXT_SUMMARY.md`：快速理解新旧系统流程、关键差异，以及去哪里看更权威的系统材料。
...
- 不熟悉项目背景：读 `PROJECT_BRIEF.md`。
- 需要稳定审计目标或原则：读 `AUDIT_CHARTER.md`。
- 需要阶段位置、gate 或路线图：读 `PLANS.md`。
- 需要快速理解新旧系统结构：读 `SYSTEM_CONTEXT_SUMMARY.md`。
...
- 不再维护单独的 `DECISIONS.md`。
- 长期性结论不写进规则文档；应写入用户后续创建的长期结论文档、对应主题记录或正式 `BK_Copilot/` 文档。
- `TASK_STATE.md` 只保留必要交接和指针，不沉淀长期结论全文。
