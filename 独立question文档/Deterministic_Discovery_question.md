...
它只做一件事：在**没有人盯着**的时候，系统自发地把当前 Rule 侧已经定义好谓词的**rule 升级候选**捞出来，投进一个审核队列，等会计师来看。它**只提候选，不落地、不审批**。
...
- automation drift / automation auto-downgrade **不再归当前确定性发现 job**。restrictive auto-downgrade 的检测、触发和可见性契约归 `Entity Log automation_policy` 的 maintenance / mutation contract，仍是 Entity Log 的 L4/seam open boundary；它可以未来复用调度器或谓词执行基础设施，但不是本机制当前职责。
...
- **批后增量**：每批交易 finalize / correction 后，只对这批动过的 entity、case pattern、rule refs 重算 Rule 侧 promotion 谓词。便宜、新鲜——抓"刚刚新增了一个可用 CaseLogEvidence / correction signal"。
...
每一类候选的"够格"谓词，归它所治理的那个记忆层。当前只有 Rule 侧 promotion 谓词：

- promotion（够不够格交给会计师看、是否可作为 rule 升级候选）谓词 → 归 **Rule Log / Rule Match 治理**。

job 只是把这些谓词**套用**一遍。判据变了，改的是各记忆层的治理，不是动 job。这样 job 永远薄、永远可单独审计。**别把谓词逻辑也塞进 job 或 Review。**

Rule 侧谓词必须和 Rule Log 的 authority-content 标准分层对齐：
...
- job 不拥有阈值数字、分布算法、结果唯一性判定、CaseLogEvidence 打包规则或 rule 被推翻累计规则；这些都归 Rule 侧定义。
...
- **Rule 侧谓词的 exact 定义** 归 Rule Log / Rule Match 治理，与本机制对齐前未冻结。
- **"rule 被推翻一次"信号** 的 source event 应来自 Human Review / Finalization 后可审计事实：原交易曾由 active rule 命中，之后经会计师纠错 append 成不同结果。信号本体不落在 job；事件事实锚在 Transaction Log correction append + rule-hit ref / Intervention trace，Case Log 保存 supersede / updated case 语义。累计 rollup、够格阈值和是否触发 inbox candidate 归 Rule 侧 open boundary。
